...
def filterList(key, value, fmt, meta):
    if fmt == 'beamer':
        fmt = 'latex'
    if fmt == 'latex':
        #latex_begin = r'\vspace{-\the\parskip}\begin{itemize}\setlength{\parskip}{1em}\vspace{-\the\parskip}\item[]'
        latex_begin = r'\vspace{-\the\parskip}'
        #latex_end = r'\end{itemize}'
        latex_end = r''
        global lastStr
        try:
            lastStr
